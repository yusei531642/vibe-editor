---
name: pullrequest
description: vibe-editor で PR を作成し、vibe-editor-reviewer (GitHub bot) の自動レビュー → 指摘修正 → 再レビュー → 品質ゲート確認 → ユーザー明示承認後のマージまで完走させる一連のワークフロー。ユーザーが「PR を作成」「プルリクエスト作成」「PR 出して」「pull request」「PR お願い」等を言ったとき、または既存 PR のレビューサイクルを回したいとき (「レビュー待つ」「指摘修正してまた送って」等) に必ずこの skill を使うこと。単純な `gh pr create` 一発で終わらせず、レビューと品質ゲートを完了し、ユーザーが承認したマージまで責任を持って見届ける workflow。
---
...
指摘の解消・品質ゲートの確認・ユーザー明示承認後のマージまでを 1 つのワークフローとして扱う。

`vibe-editor-reviewer` はレビューと判定を行うが、PR を自動マージしない。reviewer の OK は
マージ承認ではない。マージには対象 PR・現在の HEAD・マージ実行についてのユーザー明示承認が必要。
...
## Step 5: 品質ゲート確認 → ユーザー明示承認 → マージ

レビューに指摘がなくなった (`## 🤖 Claude Auto-Review` 本文に 🔴/🟡 マーカーが無い、
または「critical な問題は見当たりません」「LGTM」相当の文言) 場合でも、
**reviewer の OK をマージ承認として扱ってはいけない**。
...
- 次をすべて確認してから、対象 PR・現在の HEAD・CI 結果・reviewer の最終判定をユーザーへ提示する。
  ...
  - マージ対象の HEAD SHA が確認済み
- 上記を提示しただけではマージしてはいけない。ユーザーがその PR と現在の HEAD の
  マージを明示承認するまで待つ。

### 承認後のマージと確認

ユーザーが明示承認したら、承認された方法で PR 経由のマージを実行する。`main` への直接
push は引き続き禁止である。

```bash
# 例: ユーザーが squash merge を明示承認した場合
gh pr merge <PR番号> --squash
gh pr view <PR番号> --json state,mergedAt,mergeCommit
```

- `state: "MERGED"` かつ `mergedAt` に値あり → 完了。ユーザーに報告する。
- `state: "OPEN"` のままなら、マージ失敗の理由を確認し、再実行の前にユーザーへ報告する。
...
- **ユーザー明示承認なしの `gh pr merge`**: reviewer の OK や CI 成功だけでは実行しない。
