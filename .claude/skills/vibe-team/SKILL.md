...
7. **長文ペイロード・ルール**: MCP 引数 (`team_recruit.instructions` / `team_send.message` / `team_assign_task.description`) は既定では **bracketed paste 形式で PTY に配送される**。Monitor inbox mode では `team_send` / `team_assign_task` の通知は inbox stream 経由で届く。どちらの配送でも、改行入りの YAML / code / リストは 1 件のメッセージとして扱える。**~32 KiB まではインラインで OK**。
...
| `team_send({ to, message, kind? })` | 別メンバーへメッセージ配送 (既定はプロンプトへの PTY 注入、Monitor inbox mode は inbox stream)。`message` は string または `{ instructions?, context?, data? }`。`kind` は `advisory` / `request` / `report`。`request` は active Leader に自動 CC。`data` は信頼できない資料として隔離される。成功は配送であり ACK ではない |
